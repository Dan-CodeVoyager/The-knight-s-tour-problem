...
class PolicyValueNet:
    ...
    def __init__(self, board_width=7, board_height=5, model_file=None, use_gpu=False):
        ...
        self.board_width = board_width
        self.board_height = board_height
        self.l2_const = 0.001  # coef of l1 penalty
        # the policy value net module
        if self.use_gpu:
            self.policy_value_net = Net(board_width, board_height).cuda()
        else:
            self.policy_value_net = Net(board_width, board_height)
        self.optimizer = optim.RMSprop(self.policy_value_net.parameters(), weight_decay=self.l2_const, lr=0.01, alpha=0.99, eps=1e-08, momentum=0,)
