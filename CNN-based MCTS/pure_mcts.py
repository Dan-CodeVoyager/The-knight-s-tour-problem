...
def rollout_policy_fn(board):
    ...
    # rollout randomly
    action_probs = np.random.rand(len(board.candidates))
    return zip(board.candidates, action_probs)
...
def policy_value_fn(board):
    ...
    action_probs = np.ones(len(board.candidates))/len(board.candidates)
    return zip(board.candidates, action_probs), 0
    ...
    def expand(self, action_priors):
        ...
        for action, prob in action_priors:
            if action not in self._children:
                self._children[action] = TreeNode(self, 0)
...
class MCTS(object):
    ...
    def _evaluate_rollout(self, board, limit=1000):
        ...
        """
        player, _ = board.get_current_player()
        for i in range(limit):
            end, winner = board.is_game_end()
            if end:
                break
            action_probs = rollout_policy_fn(board)
            max_action = max(action_probs, key=itemgetter(1))[0]
            board.execute_move(max_action)
        else:
            # If no break from the loop, issue a warning.
            print("WARNING: rollout reached move limit")
        if winner is False:  # tie
            return 0
        else:
            return 1
    ...
    def get_move(self, board):
        ...
        return max(acts, key=lambda act_node: visits)
